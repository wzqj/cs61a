...
def limited(x, z, limit):
    ...
    # Both invert and change divide the 2nd parameter by x, which causes error when x = 0
    # So factor out the division by x to here where we can check x != 0 first
    if x != 0:
...
def hailstone(n):
    ...
    7
    """
    # Start with 1 since n itself counts as length of 1
    length = 1
    print(n)
    while n != 1:
        if n % 2 == 0:
